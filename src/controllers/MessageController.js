...
class MessageController {
   ...
   * @desc Obtiene mensajes de una conversación específica
   * @access Private
   */
  static async getMessagesByConversationId(req, res) {
    ...
    try {
      req.logger?.info('🔄 Iniciando búsqueda de mensajes por conversationId', {
        category: 'MESSAGE_QUERY_START',
        conversationId: req.query.conversationId,
        userId: req.user?.id,
        userEmail: req.user?.email,
        ip: req.ip,
      ...
      const { conversationId, limit = '50', cursor = null } = req.query;

      // Validar conversationId
      if (!conversationId) {
        req.logger?.warn('❌ ConversationId no proporcionado en query', {
          category: 'MESSAGE_VALIDATION_ERROR',
          userId: req.user?.id,
          query: req.query
        });
      ...
      req.logger?.debug('🔍 Validando formato de conversationId para mensajes', {
        category: 'MESSAGE_VALIDATION',
        conversationId: conversationId,
        length: conversationId.length,
        pattern: /^conv_/.test(conversationId)
      });

      // Validar límite
      const limitNum = Math.min(100, Math.max(1, parseInt(limit, 10) || 50));

      req.logger?.info('📊 Parámetros de consulta validados', {
        category: 'MESSAGE_QUERY_PARAMS',
        conversationId: conversationId,
        limit: limitNum,
        hasCursor: !!cursor
      });

      // Buscar mensajes en Firestore
      req.logger?.database('query_started', {
        operation: 'messages_by_conversation_id',
        conversationId: conversationId,
        limit: limitNum
      });

      const messages = await Message.getByConversationId(conversationId, limitNum, cursor);
      ...
      req.logger?.info('✅ Mensajes obtenidos exitosamente', {
        category: 'MESSAGE_QUERY_SUCCESS',
        conversationId: conversationId,
        messageCount: messages?.length || 0,
      ...
      res.json({
        ...
        messages: messages || [],
        pagination: {
          hasMore: messages && messages.length === limitNum,
          nextCursor: messages && messages.length === limitNum ? messages[messages.length - 1].id : null
        }
      });

    } catch (error) {
      req.logger?.error('❌ Error al obtener mensajes por conversationId', {
        category: 'MESSAGE_QUERY_ERROR',
        conversationId: req.query.conversationId,
        userId: req.user?.id,
        userEmail: req.user?.email,
        error: error.message,
