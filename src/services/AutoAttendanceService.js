...
class AutoAttendanceService {
  ...
  /**
   * Procesa la asistencia automática para todos los empleados activos
   */
  static async processDailyAttendance(date = null) {
    try {
      const targetDate = date || new Date().toISOString().split('T')[0];
      console.log(`🔄 Procesando asistencia automática para: ${targetDate}`);
      ...
      console.error('❌ Error en procesamiento automático de asistencia:', error);
      throw error;
    }
  }
  
  /**
   * Procesa la asistencia para un empleado específico
   */
  static async processEmployeeAttendance(employee, date) {
    try {
      ...
      if (existingRecord) {
        console.log(`📝 Registro existente para ${employee.personalInfo.firstName} en ${date}`);
        return {
          status: 'exists',
          action: 'skipped',
          created: false,
      ...
      if (!isWorkingDay) {
        console.log(`🏠 ${employee.personalInfo.firstName} no trabaja los ${this.getDayName(date)}`);
        return {
      ...
      console.log(`✅ Registro creado para ${employee.personalInfo.firstName} en ${date}`);
      return {
        status: 'created',
        action: 'created',
        created: true,
      ...
      console.error(`❌ Error procesando asistencia de ${employee.personalInfo.firstName}:`, error);
      throw error;
    }
  }
  
  /**
   * Crea un registro de asistencia automático
   */
  static async createAutoAttendanceRecord(employee, date) {
    try {
      ...
      const attendanceData = {
        employeeId: employee.id,
        date: date,
        clockIn: workingHours.startTime,
        clockOut: workingHours.endTime,
      ...
      console.error('❌ Error creando registro automático:', error);
      throw error;
    }
  }
  
  /**
   * Verifica si es día laboral para el empleado
   */
  static isWorkingDay(employee, date) {
    ...
    // Verificar días laborales por defecto (Lunes a Viernes)
    return dayOfWeek >= 1 && dayOfWeek <= 5;
  }
  
  /**
   * Obtiene las horas de trabajo para el empleado en una fecha específica
   */
